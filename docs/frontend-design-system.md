...
### 2.2.1 地图 token 落地（13A-1）

6 个 `ros-*` 是**原值搬迁**，不是重新取色 —— 它们是对着点云栅格与 lanelet 叠加层手调出来的，
`canvas` 与 `free` 之间那一点明度差决定的是"可通行区域看不看得出来"。重取一遍属于改地图外观，
而地图要到 13A-2 才在屏幕上；看不见渲染结果就不该定。与 `chart-1…8` 一样保持字面量还有一个
具体原因：点云栅格化是往 `ImageData` 里逐像素写值，canvas 与 zrender 都自己解析颜色，两者都
不认识 `oklch`。

`map-grid` / `map-scale` 取自 ramp，但**不进 6.1 那张审计表** —— 它们画在 `ros-canvas` 上，而
`ros-canvas` 不是语义表面 token，混进按 4.5:1 判定的表里只会用错的标准误报。改由
`docs/tools/check-map-contrast.mjs` 单独机检，**两者标准不同，这是 WCAG 1.4.11 的适用范围
决定的，不是宽严之分**：
...
### 2.2.2 点云类别色与 alpha（13A-2b）

点云背景的 obstacle / floor 两色**总是带透明度画的**，所以不能单独判对比度 —— 透明色的对比度
取决于它叠在什么上面。叠的东西是确定的（`ros-canvas`），alpha 也是确定的，所以按真实 alpha 合成
后再判，并且**两类之间也要够分**：obstacle 与 floor 编码的是"撞得到"与"开得过"两件不同的事，
把它们混起来才是真正的失效模式。

| 配对              | 下限    | 实测（浅 / 深） |
| ----------------- | ------- | --------------- |
| obstacle × canvas | **3:1** | 4.52 / 10.00    |
| floor × canvas    | 1.3:1   | 1.34 / 2.24     |
| obstacle × floor  | **3:1** | 3.37 / 4.46     |

**alpha 是调色板的一部分，这是机检逼出来的结论而不是设计口味。** 浅色第一版按深色的 alpha
下限（164）试，三组全 FAIL，而原因不是色相：在近白画布上，**64% 不透明度的洗色无论取什么颜色
都到不了 3:1** —— 剩下 36% 透出来的画布本身就把合成亮度垫在了 3:1 允许的上限之上。所以浅色
obstacle 的 alpha 下限提到 220。alpha 因此也是 token（`--ros-cloud-*-alpha`，不带 `--color-`
...
（已落地：`frontend-next/test/tokens.test.ts`。）

### 6.2.1 地图图形对比度：脚本机检（13A-1）

`node docs/tools/check-map-contrast.mjs` —— `map-grid` / `map-scale` × `ros-canvas` × 双主题四组，
下限按各自用途（见 2.2.1）。它读**生成后的真实 token** 而不是把值再抄一遍，抄一遍只是在验证
抄得对。

两次踩到同一个坑值得记下来：解析 `semantic.css` 时按裸字符串找 `@theme`，会命中文件头注释里
那段**带花括号的示例 CSS**，于是"块体"读成了注释的一部分 —— 症状是"token 缺失"而文件里明明有。
`tokens.test.ts` 的注释里已经写着它第一次运行时就是这么错的，这个脚本又错了一次。正确写法是
匹配"选择器 + 紧跟的花括号"（`/^@theme\s*\{/m`），或者先剥掉注释。
