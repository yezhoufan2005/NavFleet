...
for (const conf of ["nginx/locations.conf", "nginx/default.conf", "nginx/tls.conf"]) {
  ...
  const upstreams = [
    ...new Set([...read(p).matchAll(/proxy_pass\s+https?:\/\/([A-Za-z0-9_-]+)/g)].map((m) => m[1])),
  ];
  for (const host of upstreams) {
    if (baseServices.includes(host)) ok(`${conf} 的上游 ${host} 是 compose 服务`);
    else bad(`${conf} 的上游 ${host} 不是 compose 服务（现有：${baseServices.join(", ")}）`);
  }
}
...
const ADDITIVE = new Set(["docker-compose.monitoring.yml", "docker-compose.backup.yml"]);
for (const f of composeFiles) {
  ...
  for (const s of svcs) {
    ...
    else bad(`${f} 引入了新服务 ${s} —— 覆盖型叠加必须复用基础服务名，否则会同时拉起两套`);
  }
}
...
// ---------------------------------------------------------------------------
for (const f of composeFiles) {
  for (const m of read(join(DEPLOY, f)).matchAll(/^\s*dockerfile:\s*(\S+)\s*$/gm)) {
    const rel = m[1];
...
for (const f of composeFiles) {
  for (const m of read(join(DEPLOY, f)).matchAll(/^\s*image:\s*(\S+)/gm)) {
    const image = m[1];
    if (/:latest$/.test(image) || !image.includes(":")) bad(`${f} 的镜像未钉版本：${image}`);
...
const declared = new Set([...envExample.matchAll(/^([A-Z][A-Z0-9_]*)=/gm)].map((m) => m[1]));
const referenced = new Set();
for (const f of composeFiles) {
  for (const m of read(join(DEPLOY, f)).matchAll(/\$\{([A-Z][A-Z0-9_]*)/g)) referenced.add(m[1]);
}
const undeclared = [...referenced].filter((v) => !declared.has(v)).sort();
if (undeclared.length === 0) ok(`compose 引用的 ${referenced.size} 个变量都在 .env.example 里`);
