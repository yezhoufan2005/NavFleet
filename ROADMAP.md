...
- [ ] **CI 完全不看 `deploy/`**。14J 引入了三条跨文件约束（`locations.conf` 的上游名必须是
      compose 里存在的服务、回滚 overlay 必须复用同一个服务名、`publish-images.yml` 的 dockerfile
      必须存在），而 CI 一条都不验。14S 的 `verify-stack.sh` 现在能验前两条，**但它没进 CI**
- [ ] **后端缺 dead-export 门禁**（14O 记的那条）。console 有 `test/dead-exports.test.ts`，后端没有
- [ ] **`prom/alertmanager:v0.28.1` 仍未用本体校验过**（14R）。`amtool check-config` 用的是
      `latest`；Docker Hub 对这个仓库连续超时，网络恢复后补
