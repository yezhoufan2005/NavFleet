...
| **下一步**     | **Phase 17 进行中（报表与数据价值，发版 1.4.0）**：17A 聚合层 + 17B 报表页与导出 + 17C-1 kiosk 认证（专用只读账号 + 长刷新期，access 仍短）均合入 main。下一步 **17C-2 大屏页**（WallView：KPI 带 + 地图 + 滚动告警，版式已确认），随后收口发版 1.4.0（负责人手动合 release PR + tag） |
...
- **下一步 17C-2（大屏页）**：WallView 建出——版式经确认 = **KPI 带 + 地图主体 + 滚动告警**（`meta.bare`
  无壳、免交互、新鲜度指示必备、底部三分之一不放要紧的、两米可读），+ UsersView 加建 kiosk 账号入口，
  多分辨率适配与烧屏规避。
