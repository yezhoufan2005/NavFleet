...
| #   | 事项                       | 状态                                                                                                                 |
| --- | -------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| 1   | BUG 修复                   | 进行中 —— 第一批见 14M，死代码见 14O，告警筛选空白下拉框见 14Q                                                       |
| 2   | 旧前端冻结、新前端启用     | ✅ 完成（14J + 14L）                                                                                                 |
| 3   | 远端库整理、Docker 整理    | ✅ 完成（14R）                                                                                                       |
| 4   | 1.1.0 完成并交负责人验收   | **验收栈已起，等负责人过目**（14R）。release PR #156（`chore(main): release 1.1.0`）号是对的，等验收通过再由负责人合 |
| 5   | 演示数据更新               | 待办（负责人 2026-09-09 明确「先不动」）                                                                             |
| 6   | 全项目扫描与一致化         | 进行中 —— 六路扫描全部回收，修复批次 14M / 14O / 14P / 14Q                                                           |
| 7   | 文档整理，删冗余与错误内容 | 进行中 —— 14K 改回假话、14P 修跑不通的命令、14R 归档 Phase 11–13（2816 → 1200 行）                                   |
...
**一个新发现，负责人需要知道**：GHCR 上的三个包**匿名拉不到**。
`docker manifest inspect ghcr.io/yezhoufan2005/navfleet-{backend,console,frontend}:latest` 全部失败。
可能是包是 private（workflow 首次创建包时的默认），也可能没发出来。这件事影响部署：
`deployment.md` 9.5 教的是从 GHCR 拉镜像，而 private 包需要在服务器上 `docker login`。**要在
1.1.0 发版前确认**，否则文档教的路走不通。我看不到 package 设置，需要负责人在网页上核对。
