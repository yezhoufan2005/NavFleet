...
- [ ] `RosSceneMap`(594) / `GpsMap`(311) 重写为新设计：地图升为页面主体、侧栏可折叠
      （现在地图只是面板里的一格）
- [ ] 补两个点云调色板 token（obstacle / floor）—— 13A-2a 刻意没定浅色那一套：那是要对着
      渲染出来的地图判的颜色，和 `map-grid` 同一个道理
- [ ] `GpsMap` 的 `useTheme()` 用法是 `{ state }`，与 frontend-next 的 `{ resolved }` 不兼容，重建时对齐
- [ ] **规模退化**（11C 决定 2）：`/devices` 首次进入的默认视图按车辆数自动选，阈值 **40 台**
      ...
      一次点击。三态「列表 / 地图 / 自动」，13A-1 刻意没搬旧的两态偏好
- [ ] 把 `dashboard.spec.ts` 接进 `SHARED_SPECS`
- [ ] e2e 契约要保住：`document.querySelector(".map-surface svg")` →
      `.ros-marker.fusion .ros-marker-core`；`.map-surface` 在视图的 CSS 里而不在组件里；
      `.ros-marker-core` 必须是以 0,0 为中心、放在位姿平移组内部的图形
