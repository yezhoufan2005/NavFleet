...
- [ ] 第 9 节那 29 条缺陷逐条标注"已修 / 仍在 / 不修及理由"（现在散落在各 PR 的 ROADMAP 条目里，
      没有一处能一眼看全）
...
- [x] 四张统计卡卡内加分解明细，全部来自客户端已有的数据，不新增接口：在线卡列**离线车名**
      （超过 3 台变成「等 N 台」——列四十个名字的卡片是没人看的卡片）、告警卡**按严重度分三档**、
      GPS 卡列**无定位的车**（用与 store `gpsCount` 完全相同的谓词，两处因此不可能对不上）
- [x] **编队卡刻意不列每个编队** —— 同一页下面的编队面板已经在列，还带描述。它没说的是
      **有几个编队满员**，而那才是可执行的部分：缺一台车的编队跑不了它的路线
- [x] **浅色模式的警示信号：用户报「数字颜色不清晰」，但量出来浅色对比度更高** ——
      ...
      修法按项目图表已在守的规则：**文字穿文字色，颜色靠旁边的标记承担** —— 大数字永远
      `text-ink`，状态交给一个 `bg-warning` / `bg-critical` 的饱和圆点，而且**没有要报的事就不点亮**
      （常亮的点等于没有点）。这也让总览与地图、设备列表、系统状态用同一种方式表达状态
- [x] 两条回归断言把这个决定钉住：任何一张卡的数字都不得带 `text-warning-ink` / `text-critical-ink`；
      该报警的卡必须有饱和色点、不该报的卡必须没有
- [x] 顺带修一处测试脆弱性：`find("dl")` 原本要找 告警摘要 面板，加了卡内 `dl` 之后它会落到
      第一张卡的分解上 —— **仍然通过，但通过的理由错了**。改成按标题定位到那个 section
- 自检 ✅（2026-08-31）：console **442** 例（新增 6）· **73 例 e2e 全绿** ·
  axe 13 路由 × 4 视口 × 双主题零违规 · 覆盖率 96.29 / 86.42 / 91.50 / 96.29（门槛未调）

#### 13R-C 告警列表抖动

- [ ] **排序键每秒都在变。** `allAlerts` 按 严重度 → `ts` 倒序排，而报码告警的 `ts` 取车端上报的
      `stamp`、**每个遥测周期刷新一次**：同一严重度桶内所有行的时间戳每秒一起跳到"现在"，
      先后由毫秒级差异随机决定 → 每秒重排一次，看起来就是闪烁。
      **这不是演示数据的问题**：真实车辆同样周期上报，同一个错误码会带着新 stamp 反复到达。
      修法是按**发生时刻**排序（稳定），需要在 store 的告警 diff 处补一个 first-seen —— 那里本来
      就在做新增/消失的比对；"最后上报"仍然显示，只是不再决定顺序
