...
| **下一步**     | **Phase 16 进行中**：16A（确认落库，#190）、16B（告警历史与统计，#192）、16C-1（规则引擎收敛+配置化，#193）、16C-2（报码字典，#194）、16D-1（外发骨架+HTTP 渠道+发送记录，#195）已合入 main。接着 16D-2（邮件/用户组/汇总-静默-升级/只读外发页/可观测性）**收口发版 1.3.0**。16A–16D 用 `feat` 累积到 release-please 的 1.3.0 PR，合并与 tag 由负责人手动 |
...
#### PR 16D-2 — 邮件 / 用户组 / 汇总-静默-升级 / 只读外发页 / 可观测性（收口发版 **1.3.0**）

- [ ] 邮件渠道（nodemailer 直连 SMTP）；钉钉「加签」逐请求签名（需第二个密钥）
- [ ] 分级路由细化：warning 汇总 + 静默窗口 + 去重加固 + 升级策略
- [ ] 收件方接到用户组（Phase 15C 的第二个职责）
- [ ] 只读控制台「外发」页：发送记录 + 生效渠道概览（消费 16D-1 的 `GET /notify/*`）
