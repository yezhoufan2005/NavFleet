...
- [x] **第一步（开发侧，本 PR）**：CI 矩阵 `[20, 22]` → `[22, 24]`、六处 `engines` 改 `>=22`、
      ...
      与 vitest 的大版本。
- [ ] **第二步（测试框架）**：`vitest` 与 `@vitest/coverage-v8` 在同一个提交里抬到 5
      （拆开必定不解析，peer 是精确版本号）。迁移面已查：`clearMocks` 默认变 `true`、
      未 await 的异步断言会判失败、coverage 的 include/exclude 改成相对路径匹配（可能挪动
      90% 门禁）、jsdom 里对 `globalThis` 的赋值会传导到真实 DOM —— 最后一条正对着
      `test/setup.ts` 里那一批全局 stub。
- [ ] **第三步（交付侧，要跑容器闭环）**：**三个 Dockerfile 现在钉的是 `node:20-alpine`，
      也就是说我们此刻交付的镜像跑在一个已 EOL、不再有安全补丁的运行时上。**
      这是本轮顺带查出的问题，与 vitest 无关。改 `node:24-alpine`（当前 active LTS），
      但它改的是交付产物，要和 compose 起停一起验，所以单独一个 PR，不夹在依赖升级里。
