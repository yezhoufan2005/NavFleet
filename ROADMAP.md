...
| PR          | 内容                               | CI        | 处置                                          |
| ----------- | ---------------------------------- | --------- | --------------------------------------------- |
| #133        | dev-dependencies 组 3 项           | 8/8       | 合入                                          |
| #134        | prod-minor-patch 组 3 项           | 8/8       | 合入（让 dependabot 先 rebase 出新 lockfile） |
| #131 + #135 | vitest / @vitest/coverage-v8 5.0.0 | 各 6/8 红 | **等一个决定，不是等一次修**                  |
| #132        | eslint 10 + typescript-eslint      | 4/8 红    | **等 flat config 与插件**                     |

- **#131 与 #135 必须一起走，而且现在走不了**：`@vitest/coverage-v8@5` 的 peer 是精确的
...
- **分支**：21 个已并入 `main` 的远端分支（14B–14F、13S/13T 系列、release-please 的陈旧分支等）
  与 12 个本地同名分支删除；提交都在 `main` 里，删的只是指针。
