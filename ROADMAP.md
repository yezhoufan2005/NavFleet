...
| **下一步**     | **Phase 15D**（审计日志）。15A ✅（#176）· 15B-1 ✅（#179）· 15B-2 ✅（#181）· 15C ✅（#183，权限矩阵）均未发版。15D：`audit_log` 集合 + 中间件、请求日志补用户名、查询接口 + TTL |
...
| 2    | **Phase 15** 用户体系与真 RBAC（1.2.0）   | 15A ✅ · 15B-1 ✅ · 15B-2 ✅ · 15C ✅ · 15D 进行中 |
...
- [ ] **日志脱敏：broker 连接串这一路没有走 pino 的 redact（`mongoUri` 那一路有）。已建议提前到
      15A 做**，见 Phase 15 开头的核对表。改动很小，但**修法不是往 `REDACTED_PATHS` 加一条路径**；
      动手前先读 `backend/src/logger.ts` 顶部那段注释，它解释了为什么那条路径是刻意不加的
      （按「先修后写」，这条只写到影响与去哪看为止）
