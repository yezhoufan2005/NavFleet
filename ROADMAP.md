...
- [ ] `docs/frontend-parity.md` 第 1–4、6–8 节逐条核销（第 5 节 ✅ 13E）
  - [x] **第 1 节 应用外壳（19 行）· 第 4 节 GPS 地图（22 行）· 第 6 节 告警中心（25 行）·
        第 7 节 设置/登录/404（30 行）** ✅（2026-08-31）。96 行里 81 行核销通过，**15 行是真丢了
        能力**，逐条列在各节末并归入 13T
  - [ ] 第 2 节 Dashboard（63 行）· 第 3 节 ROS 场景地图（89 行）· 第 8 节 全局机制（53 行）
...
### 13T — parity 第 1/4/6/7 节核销查出的 15 处能力损失

96 行里 81 行核销通过，**15 行是真的丢了能力**。它们有一个共同点，值得单独写下来：
**丢的几乎全是旧实现里那些防御性的小东西** —— 一条 `pointer-events:none`、一次 z-index 抬升、
一个重试按钮、一个徽标、一个计数。主干功能在搬迁时被认真对待了，边角没有。

**最有力的检测手段是"死导出"。** 核销中发现 5 个有导出、零非测试消费者的键，
**每一个都精确对应一个没建出来的 UI**：

| 死导出                                                         | 对应缺失的 UI            |
| -------------------------------------------------------------- | ------------------------ |
| `retryBootstrap`（`stores/fleet.ts:785`）                      | 离线重试按钮             |
| `cycleTheme`（`useTheme.ts:116`）                              | header 主题快捷入口      |
| `acknowledgedCount`（`useAlertAck.ts:74`）                     | 「显示已确认（N）」的 N  |
| `clearAll`（`useAlertAck.ts:68`）                              | 「清除已确认」控件       |
| `clearSavedSceneViews`（`useSceneViewportPersistence.ts:113`） | 「清除场景视图记忆」控件 |

这不是巧合：**搬 store 的时候把逻辑搬全了，搬界面的时候漏了调用它的那个控件。**
所以「导出但无人消费」是一条可机检的 parity 缺口指纹，13T 之后应当考虑把它做成断言。
