...
**负责人 2026-09-09 定的发版前七件事**，这是当前的工作序列。原话是「在完成〔七件事〕**后再发版
再规划后续**」，所以**发版是第八步，不属于任何一件**：

> **七件全部做完 → 才发版 → 才规划后续。** 第 4 件是「1.1.0 完成并**交负责人验收**」，验收 ≠ 发版。
> **#156 已于 2026-09-09 由负责人指示关闭**，指令是「删除 #156，先不发版 1.1.0」。注意关闭
> **只能推迟不能取消**：产生它的 `feat` / `fix` commit 还在 main 上，下一次 push 到 main 时
> release-please 会用同样的内容重建一个新号的 PR。这条是本仓库踩过的 —— 1.1.0 曾被误发一次
> （#78 关闭 → 重建为 #83 → 被合 → tag/Release/镜像全部产生 → 负责人手动删除）。
> 我还曾在一次汇报里把「合 #156」列进「待你处理」，那是把第八步提到了第五件之前。两件都记在
> 这里，免得再错。
...
| 5   | 演示数据更新               | ⏸ **未做**，负责人明确「先不动」—— 这是当前唯一挡在发版前面的一件                                     |
...
- [ ] 发版 **1.1.0** —— **负责人两次明确暂缓，第二次是直接关掉 release PR #156**（2026-09-09）。
