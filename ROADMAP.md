...
- **总览四张卡的卡内分解明细**（离线车名 / 严重度三档 / 无定位车 / 编队满员）——设计意图已在「当前真相」留档。
- ~~主题样式（P2）~~ → **已由 `v1.5.0` 焕新收口**。
- **演示数据更新**（`config-runtime/*.json` + `e2e/support/seed.ts` + `backend/scripts/mock-mqtt.ts` 三处联动）
  ——负责人「先不动」。
