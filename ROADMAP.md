...
| 项目           | 现状                                                                            |
| -------------- | ------------------------------------------------------------------------------- |
| **已发布版本** | 1.0.3（`.release-please-manifest.json`）。1.1.0 **未发布**，负责人明确暂缓      |
| **部署的前端** | `frontend-next/`（workspace `navfleet-console`），compose 服务名 `web`          |
| **旧前端**     | `frontend/` **已冻结**：代码全留、不再改动，`--legacy` / 回滚 overlay 可启用    |
| **发布的镜像** | `navfleet-backend`、`navfleet-console`。`navfleet-frontend` 停在 1.0.x          |
| **CI**         | 5 个 job：backend+shared、fleet-core+console、frozen frontend、e2e、GitGuardian |
...
> **本阶段的 13 个子批次分两条线，而它们在文件里不按顺序排。** 这一节是索引；现状看开头的
> 「当前真相」。
>
> 之所以要索引：**14C–14I 物理上排在 Phase 13 的段落里**（第 1222–1520 行附近），因为它们是
> 在 13 的收口过程中一轮轮追加的；14A/14B/14J–14M 排在本节之后。**没有把它们搬到一起**是刻意
> 的 —— 每一批都记着当时的实测数字与判断依据，搬动 300 行只为让目录整齐，换来的是一次不必要
> 的、无法逐条复核的大改。索引解决的是「找不到」，而那才是真问题。
>
> **人工验收线**（负责人逐轮提出、我逐条落地）：
>
> | 批次      | 内容                     | 位置        |
> | --------- | ------------------------ | ----------- |
> | 14A       | 第一轮，6 条             | 本节之后    |
> | 14C       | lede 清理 + 设备列表排序 | Phase 13 段 |
> | 14D       | 统计卡警示色改色块承担   | Phase 13 段 |
> | 14E       | 第二轮，6 条             | Phase 13 段 |
> | 14F / 14G | 第三、四轮，11 条        | Phase 13 段 |
> | 14H       | 第五轮，5 条             | Phase 13 段 |
> | 14I       | 第六轮，5 条             | Phase 13 段 |
>
> **工程动作线**（切换本身与它的连带工作）：
>
> | 批次 | 内容                                    | 位置     |
> | ---- | --------------------------------------- | -------- |
> | 14B  | 两侧性能对比（同机同次会话）            | 本节之后 |
> | 14J  | compose 切换到 v3 控制台                | 本节之后 |
...
> | 14Q  | 新前端死表面：8 个 token 一个不删的量测 | 本节之后 |
