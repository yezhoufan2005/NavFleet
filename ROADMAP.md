...
**一个停在远端的分支，是刻意留的不是垃圾**：`fix/console-review-round1-overview`
（`567c7a3`，2026-08-31，1 个 commit）。它就是上面搁置的第一项 —— 四张统计卡的**卡内分解明细**：
在线卡列出离线车名（超过 3 台变成「等 N 台」）、告警卡按严重度分三档、GPS 卡列出无定位的车、
编队卡说有几个编队满员。

**它没有被后来的改动取代。** main 上每张卡只有一行汇总 note（「1 台离线」「2 条告警级」
「2 台无定位」），`满员` 与「等 N 台」只存在于这个分支。所以它是一份**尚未被采纳的设计提案**，
不是已完成工作的残留。

代价说清：它落后 main 九天，`OverviewView.vue` 这九天里被改过，重新启用需要 rebase；提交信息里
的「console 442 例」也已经变成 558。要么负责人看过之后拍板采纳（我 rebase 并补测），要么明确
不采纳（我删分支，设计意图留在这段文字里）。
