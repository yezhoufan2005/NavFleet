...
| **实测基线**   | 单测 **1266**（fleet-core 123 · backend 453 · 冻结前端 132 · console 558）· E2E **80**（`retries: 0`，已三轮零抖动）· 四个覆盖率门槛全部通过，余量一致地留 2–3 个百分点                                                             |
| **下一步**     | **Phase 15E-2**（控制台管理页 + **发版 1.2.0**）。15A–15D + **15E-1（逐会话追踪 + 账号锁定，#187）** 均 ✅ 未发版。15E-2：用户/审计/会话管理页 + 个人中心改密、`feat:` + `Release-As: 1.2.0` —— **发版 PR 合并与 tag 由负责人手动** |
...
| 2    | **Phase 15** 用户体系与真 RBAC（1.2.0）   | 15A ✅ · 15B-1 ✅ · 15B-2 ✅ · 15C ✅ · 15D ✅ · 15E-1 ✅ · 15E-2 待办（发版） |
...
#### PR 15E-2 — 控制台管理页（**发版 1.2.0**）

- [ ] 管理页：用户 / 审计日志 / 会话（列表 + 强制下线）；个人中心：改密 / 改显示名 / 本人会话下线
- [ ] 发版 **1.2.0**（`feat:` + `Release-As: 1.2.0`；发版 PR 合并与 tag 由负责人手动）
