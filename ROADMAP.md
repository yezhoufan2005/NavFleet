...
七件事收口于 1.1.0 发版。之后的顺序是定好的，**不要跳**：

| 顺序 | 内容                                    | 为什么在这个位置                                                         |
| ---- | --------------------------------------- | ------------------------------------------------------------------------ |
| 1    | **P0-f 工程门禁批次**（8 项）           | 见下。它的性质是「打开新规则再修报出来的东西」，量级不可知，必须独占一段 |
| 2    | **演示数据更新**                        | 七件事里唯一被暂缓的一件，发版后重新排                                   |
| 3    | **Phase 15** 用户体系与真 RBAC（1.2.0） | 15A schema 迁移机制必须先做 —— 后面每个 PR 都要改集合结构                |

**P0-f 为什么必须独占一段**：它包含 `recommendedTypeChecked`、`--max-warnings 0`、
`noUncheckedIndexedAccess`，以及两个必须手写的大版本升级（eslint 10 + plugin-vue 10、
vitest 5 + coverage-v8 5，后者还要重定四个覆盖率门槛）。前三项都是**打开一批新规则、然后修它们
报出来的东西**，报多少条事先不知道；后两项被 dependabot 的「半个升级」卡着（#131 只改 vitest、
#135 只改 coverage-v8，两个都不能单独合）。放在任何一次发版之前，都会让那次发版的内容取决于一批
还没跑过的规则。

**搁置项不进这个序列**：`frontend-next` 改名、总览四张卡、主题样式 P2、告警区布局。它们要做，
但由负责人在上面三件之间插入，我不自行开工。
