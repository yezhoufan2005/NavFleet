...
| **下一步**     | **Phase 17D（象限 B 收尾 / 数据利用清零）已开工**，见下「剩余未完成任务总览」与 Phase 17D 小节。其后是 **Phase 18（交付成熟度收尾）**；按负责人 2026-09-09 定的顺序推进                                                                                                                                   |
...
**「最后三项」（总览四张卡、主题样式、演示数据）已于 2026-09-20 全部收口**（做完或定为不做），
详见下面「最后三项 —— 已收口」一节。仍开放的长期搁置只剩 `frontend-next` 改名与告警区布局两项。
...
- **Phase 17D — 象限 B 收尾（数据利用清零）**：v1.5.0 发版后按顺序推进的第一项，已开工。见下方 Phase 17D 小节。
...
- ~~总览四张卡的卡内分解明细~~ → 收口：当前每卡一行汇总即够用，不再做卡内分解。
- ~~主题样式（P2）~~ → 已由 `v1.5.0` 焕新收口。
- ~~演示数据更新~~ → 收口：负责人「先不动」，不再作为待办。

### 长期搁置（性质不同，不属于「最后三项」）

- **P1 多厂商 / 多车型适配**（展开时机待定）。
- **`frontend-next` 改名**（要动 CI / compose / playwright / lockfile，等整理完再谈）。
- **告警区整体布局**（负责人只说「可以再看看」，无具体诉求）。
- **P0-f 九条里唯一未做的一条**（jsdom 30 相关，见下方 P0-f 小节，阻塞已换形状）。

### 建议推进顺序

1. ~~**`v1.5.0` 焕新**验收 → 合并 → 发版~~ ✅ **已发版（2026-09-21）**。
2. **Phase 17D**（范围小，清一批「数据接了没人读」的欠账，收尾干净）—— **当前进行中**。
...
| 2    | **Phase 15** 用户体系与真 RBAC（1.2.0）   | ✅ 已发版 `v1.2.0`                                                                          |
| 3    | **Phase 16** 告警体系深化（1.3.0）        | ✅ 已发版 `v1.3.0`                                                                          |
| 4    | **Phase 17** 报表与数据价值（1.4.0）      | ✅ 已发版 `v1.4.0`（17A 聚合层 · 17B 报表页/导出/定时邮件 · 17C kiosk 认证 + 大屏页）       |
| 5    | **Phase 18** 交付成熟度收尾（2.0.0）      | 待办                                                                                        |
| 6    | **最后三项**（见下一节）                  | ✅ **已收口（2026-09-20）**：主题样式随 `v1.5.0`；总览卡明细与演示数据由负责人定为不做/够用 |
...
**六批全部完成之后回头看，这批工作真正的产出不是「门禁更严了」，而是门禁**查出来**的东西：**

- 第 1 批：一条抑制着 typescript-eslint v8 里**已不存在**规则的 `eslint-disable`
- 第 2 批：`pointAtFraction` 的返回类型写 `Point`、空数组时实际返回 `undefined`；`pointOnRoute`
  在空 route 上抛异常
- 第 3 批：**9 个真缺陷**，包括 `websocket.ts` 在 `Buffer[]` 分支上解码错误、两个场景路由
  `await` 同步方法并把测试替身带歪（同步化后暴露出一处真的假绿）、两个归一化层的
  `String(unknown)` 会把对象渲染成 `"[object Object]"` 送进 UI，以及**规则看不见的那一半**
  （7 处 `as string`，是写测试时才发现的）
- 第 5 批：console 的测试 import `node:fs` 而 workspace 从未声明 `@types/node`；以及**四个覆盖率
  门槛里有一部分从来不是真实断言**（冻结前端 `functions: 81` 量的是 v8 对未被 import 文件的虚假
  100%）
- 第 6 批：`persistence.ts` 在生产里跑的那一半**一条测试都没有**

**一个门禁的价值等于它查出来的东西，而不等于它的严格程度。** 这也是为什么其中两处是「按目录把
规则关掉」而不是改代码 —— 说清是规则不适用（测试替身的 `require-await`）还是工具链答不了
（typescript-eslint 不加载 Vue 的 TS 插件），比让 45 处噪音留在报告里更有用。
...
那一半 —— 实测还缺 `@eslint/js` 10 与 `typescript-eslint` 8.70，一共四件。**14AB 手写完成后，
#131 / #135 / #139 已逐个写明理由并关闭。**

## 最后三项 —— 已收口（2026-09-20）

原由负责人 2026-09-09 定为「后置到所有任务之后」，2026-09-20 负责人决定三项均收口（做完或定为不做），
**不再是待办**。存档如下：

| 项                 | 结果                                                                                                                                |
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------------- |
| **总览四张统计卡** | **不做**：当前每卡一行汇总 note 即够用，不再做卡内分解明细（离线车名 / 严重度三档 / 无定位车 / 编队满员——设计意图存档于此以备将来） |
| **主题样式**（P2） | **做完**：由 `v1.5.0` 焕新收口（GitHub Primer 明暗两套 + 组件一致性）                                                               |
| **演示数据更新**   | **不做**：负责人「先不动」，`config-runtime/*.json` / `e2e/support/seed.ts` / `backend/scripts/mock-mqtt.ts` 三处维持现状           |

另有两项搁置、性质不同（长期搁置，不属于「最后三项」，仍开放）：
...
> **2026-09-10 勾选核对**：下面这份清单实际有 **9 条**（立项时说的「八项」少算了后来补进来的 jsdom
> 那条），其中 **8 条已完成**，而它们的复选框此前还全是 `[ ]` —— 开头「当前真相」说六批全完成，
> 这一节说全没做，正是「同一件事在两个段落里两种说法」那个毛病本身。已逐条对齐，并注明落在哪一批。
> **唯一仍未做的是最后那条 jsdom**，它不在六批之内。

- [x] ESLint 启用 `recommendedTypeChecked`（**14Z**，第 3 批）—— 立项时三份配置全是 `recommended`，
      `no-floating-promises` / `no-misused-promises` / `await-thenable` **全不生效**，而 P0-a 与 P0-b
      恰是这个规则集专抓的那一类（async 事件回调、被丢弃的 Promise）。**这条的判断后来被证实**：
      164 处报错里挑出 9 个真缺陷，是六批里命中率最高的一批
- [x] `--max-warnings 0`（**14X**，第 1 批）—— 立项时 `no-unused-vars` 三处都降为 `warn` 且全仓无
      `--max-warnings`，warning 永不让 CI 红，等于没配。**刻意排在最前面**：它一旦立起来，后面每批
      新开的规则就不可能以 warning 的形式偷偷积压
- [x] Playwright CI `retries: 1` → 0（**14X**，第 1 批），flaky 不再被重跑掩盖。**这条当时已有实测
      依据**：#79 那次抖动是一个真的对比度缺陷（过渡中间态 1.38:1），而重试对它完全无效 —— 重跑
      同一段"导航→审计"只会再落进同一个窗口，`retries` 唯一的作用是把真缺陷标成 flaky 然后放行。
      详见执行记录 2026-08-29 条。**落地后已跑过三轮 80 例，零抖动**
- [x] `packages/shared` 纳入 lint（**14X**，第 1 批；立项时被根 `eslint.config.mjs:23` 显式 ignore）。
      纳入后查出的东西只有一条，但很说明问题：一条 `eslint-disable` 抑制着 typescript-eslint v8 里
      **已不存在**的规则。**「e2e 纳入 `format:check`」这半条已经不成立** —— 根 `format:check` 里
      已经有 `prettier --check "e2e/**/*.{ts,json}"`，是这条待办写下之后补的，核对时才发现，已划掉
...
- [x] tsconfig 补 `noUncheckedIndexedAccess` 等严格开关。**这条待办的括号里原本写着「四份配置现在
      ...
      改它的那一个」
- [x] **eslint 9 → 10 + eslint-plugin-vue 9 → 10，必须同时，而且要手写 PR。**（**14AB**，第 4 批；
      手写时实测**一共四件** —— 还要 `typescript-eslint` 8.70 与 `@eslint/js` 10）dependabot 分开开了两个
      ...
      **2026-09-09 复核：同一个形状又来了一遍。** 当前挂着的是 **#139（eslint 9.39.5 → 10.10.0）**，
      ...
      结论不变，只是 PR 号换了
- [x] **vitest 3 → 5（含 `@vitest/coverage-v8`），必须同时，且要重定覆盖率门槛。**（**14AB**，第 5 批；
      「需要重新测量」这个判断成立得比预想更彻底 —— 见下面「四个覆盖率门槛全部重定」）历史：#107 单独抬
...
- [~] **jsdom 26 → 30 是取舍而不是升级。** #109 只在 **node 20** 红、node 22 绿（console 与 frontend
  ...
  **2026-09-10 复核：vitest 5 已经落地（第 5 批），而 jsdom 没有跟着抬 —— 全仓仍是 `^26.1.0`。
  这是 P0-f 九条待办里唯一没做的一条，且它的阻塞换了形状，不再是原来那个决定。** jsdom 30.0.1 的
  `engines` 是 `^22.22.2 || ^24.15.0 || >=26.0.0`，而根 `package.json` 的 `engines` 是
  `^22.13.0 || ^24.0.0 || >=26.0.0` —— 差的不再是「要不要放弃一整个 Node 大版本」，而是
  **要不要把 22.x 的下限从 22.13 抬到 22.22.2**（24.x 同理 0 → 15）。CI 矩阵取的是 `node-version: 22`
  的最新补丁号，早就满足；真正被这个下限约束的是**部署侧允许装什么 Node**，所以仍然是一次
  「缩小支持面」的决定，只是代价小了一个数量级。**归入依赖维护，不再算工程门禁的欠账。**
...
**开工前的前提核对（2026-09-10）**：下面这五个 PR 的清单写于 2026-08-29，此后代码走了 12 天、
两个前端切换过一次、六批工程门禁改过 300 多处。所以在动手前把每条待办依赖的**事实**重新量了一遍，
免得照着一份已经不成立的现状表施工。结论是**五条前提全部仍然成立**，但有三处描述需要更正：

| 前提                                 | 复核结果                                                                                                                                                                        |
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 无任何迁移机制                       | ✅ 成立。`backend/src` 与 `packages/*/src` 里 `migrat` / `schemaVersion` **零命中**，无 migrations 目录                                                                         |
| `UserRecord` 只有 5 个字段           | ✅ 成立，但**位置变了**：现在在 `backend/src/types.ts:23-29`（曾在 `packages/shared`，因为「密码哈希的类型不该待在两个前端都 import 的包里」而搬走）。15B 要改的是 backend 那份 |
| `requireRole` 只有 1 处调用          | ✅ 成立，唯一调用点是 `backend/src/routes/debug.ts:16`。**「扩展到全量」的分母是 8 条业务路由**：scenes 3 条、fleet 4 条、debug 1 条（auth 4 条与 ops 4 条另算）                |
| 无 `tokenVersion` / 无账号级登录锁定 | ✅ 成立。注意 grep `failedAttempts` 会命中 `mongoConnection.ts` 的重连计数 —— **同名不同义**，别当成已有实现                                                                    |
| 无 `audit_log`                       | ✅ 成立。但 `audit` 这个词已被占用：`startupChecks.ts` 的 `auditProductionConfig` 是**启动期配置审计**，与 15D 的操作审计无关，命名上要避开                                     |

另外两处与 Phase 15 相关的更正，都记在对应 PR 下面了：15A 的 TTL 行号已漂移、且那个坑有**没被记下
的另一半**；15D 的日志脱敏缺口仍在，但**修法不是往 `REDACTED_PATHS` 加一条路径** —— 原因写在
`backend/src/logger.ts` 顶部注释里（那段注释本身是对的，问题出在第二个调用方复用了它刻意留的例外）。
**建议把这条从 15D 提前到 15A 一起做**：它是个很小的改动，而排在 15D 意味着它要多留三个 PR 的时间。

**核对时看清的另一件事，是这个阶段的真实起点**：三个角色的**名字**到处都在（`UserRole` 类型、
种子管理员、会话菜单里的 `ROLE_LABELS`），但按角色做判断的地方就是 15C 待办里说的那一处。前端
`router.beforeEach` 挂的 `createAuthGuard` 只判**登录与否**，13 条路由记录的 `meta` 里只有 `bare`
与 `title`，没有 `roles`。所以 Phase 15 不是「把现有 RBAC 做细」，而是**从一处扩到全部**；
README 里那句 RBAC 表述也正因此需要按 15C 的待办改掉。

**另有一个交付层面的决定要负责人先定，内容不写在这里**（按「先修后写」，它涉及一个尚未处置的
配置项在 RBAC 落地后的含义变化）—— 已在会话里单独提出，定了之后再决定是记进本文件还是直接改掉。

**迁移测试用什么跑，也建议先定。** 15A 的迁移脚本必须能被测试，而本仓库刻意没有「测试要连真
mongod」这条依赖。14AA 留下的 `__setDbForTests` + 假 `Db`（`backend/test/persistence-mongo.test.ts`）
...
### PR 15C — 权限矩阵 ✅ 完成（#183，`c84d2c8`，未发版）

**范围收敛（探索发现 + 负责人确认）**：只读系统、配置走文件监听不走 API，**没有 operator 专属
后端路由**，真实矩阵很薄（读=viewer+，`/users*` 与 `/debug/ingest`=admin）。所以**用户组与权限集
抽象延后到有消费方**（通知收件方那半属 16D），本 PR 只把薄矩阵显式化 + 测试 + 前端门禁 + 文档。

- [x] 权限矩阵**钉成契约**：`backend/test/http-rbac.test.ts` 每路由×每角色网格（读接口
      viewer/operator/admin 均可达；admin 路由对 viewer/operator 403；匿名全 401）。路由代码不改
      （本就正确分层），不给读接口加冗余的 `requireRole("viewer")`
- [x] 前端：路由 `meta.roles` + `NAV_SECTIONS.roles`；`createAuthGuard` 越权重定向到 landing；
      `AppSidebarNav` 非 admin 隐藏「管理」入口（`/admin/*` 仅 admin，负责人定）。隐藏是 UX、守卫是门禁
- [x] 三角色边界写进文档：README RBAC 段改写为真实矩阵，**operator 暂 ≡ viewer**（其操作面随 16A）；
      接口表补 `/api/users*` 与 change-password
- [~] 用户组：**延后**（无消费方，非本阶段）
- 自检：backend 412 · console 562 · 覆盖率 88.14/77.18/88.07/88.37 与 92.06/82.86/90.93/94.31 全过 ·
  e2e 80/80 · CI 9 job 全绿

### PR 15D — 审计日志 ✅ 完成（#185，`345496c`，未发版）

- [x] `audit_log` 集合（`ensureMongoCollections` 建，不走迁移）+ 埋点：登录/登出/登录失败、
      自助改密、管理员重置密码、用户增/删/改。**范围与负责人确认**：只记鉴权 + 用户管理事件，
      **不记**配置热加载（无操作者）与读请求（只读系统里是噪声）
- [x] 请求日志补 `username`（`authenticate` 在 finish 前已置 `request.user`）
- [x] 查询接口 `GET /api/audit`（admin-only）+ `auditQuerySchema` + OpenAPI + TTL
      （`AUDIT_RETENTION_SECONDS` 默认 365 天，`reconcileTtls` 收敛）。**管理页归 15E**
- [x] 日志脱敏那半（broker URL）**已在 15B-1 提前做掉**
- 自检：backend 427 测试 · 覆盖率 87.79/76.81/87.50/87.97 全过 · e2e 80/80 · CI 9 job 全绿

### PR 15E — 会话管理与管理端 UI（**发版 1.2.0**）

**拆两个 PR**：15E-1 后端（本阶段唯一再次改动 token 模型，风险最高，单独过 CI），
15E-2 控制台管理页并收口发 1.2.0。

#### PR 15E-1 — 逐会话追踪 + 账号锁定 ✅ 完成（#187，`2240d9f`，未发版）

- [x] token 加 `sid` claim + `sessions` 集合（TTL on `expiresAt`，`{username,createdAt:-1}` /
      唯一 `{sessionId}` 索引）；每请求 + WS 握手在 enabled/tokenVersion 之外再查 `sid` 仍活跃。
      旧无-`sid` token 仍按 tokenVersion 治理，不强制重登（首次 refresh 升级为受追踪会话）
- [x] 语义调整：**logout 只吊销当前会话**（不再 bump tokenVersion）；改密/禁用/改角色/重置
      仍全设备失效（bump + 清空会话行，避免会话列表残留幽灵行）
- [x] 本人会话可见性 + 下线：`GET /api/auth/sessions`、`DELETE /api/auth/sessions/:id`；
      管理员强制下线：`POST /api/users/:username/logout`（不禁止对自己，但记审计）
- [x] 账号级登录锁定：`UserRecord` 加 `failedAttempts`/`lockedUntil`，**迁移 v3** 幂等 backfill；
      连续失败达 `AUTH_LOCK_THRESHOLD`（默认 5）锁 `AUTH_LOCK_WINDOW_MS`（默认 15 分钟），
      锁定期内正确口令也拒且不泄露锁定状态，过期计数清零；与每 IP 限流正交
- [x] `AuditAction` 扩 `session_revoke`/`force_logout`/`account_locked` 并埋点
- 说明：`sessions` TTL 用 expire-at-date（`expireAfterSeconds:0` on `expiresAt`），窗口写在
  `expiresAt` 里由 `JWT_REFRESH_TTL` 计算，故 `reconcileTtls` 无需为它加对账分支
- 自检：backend 453 测试 · 覆盖率 86.79/76.39/86.25/86.94 全过 · e2e 80/80 · CI 9 job 全绿

#### PR 15E-2 — 控制台管理页 ✅ 完成（#188，`bf9a9ba`，收口 1.2.0）

- [x] 后端补管理员会话端点：`GET /api/users/:username/sessions`、`DELETE /api/users/:username/sessions/:sessionId`（复用 15E-1 服务方法）
- [x] 用户管理页（`/admin/users`）：增删改、改角色、启停用、重置密码、强制下线；行内展开查看某用户会话并逐个下线。会话按 user 维度折进用户页，未做独立 `/admin/sessions`
- [x] 审计页（`/admin/audit`）：服务端筛选（操作者/动作/时间）+ 客户端分页 + 空/错态
- [x] 个人中心（`/profile`，viewer+）：改密 + 本人会话下线（下线当前会话即登出）；会话菜单加入口
- [x] `UiConfirmDialog`（reka AlertDialog）+ 恢复 `UiButton` danger 变体 + `--color-critical-hover` token；`fleetApi` 十个管理方法（失败携带后端错误码）+ `useAuth` 改密/本人会话
- [x] `feat:` + `Release-As: 1.2.0`：release-please 会生成 1.2.0 发版 PR —— **合并与 tag、关联 issue 由负责人手动**
- 自检：console 91.8/83.9/90.0/94.0 · fleet-core 96.9/82.9/92.3/96.7 · backend 86.8/76.4/86.3/87.0 · 冻结达标 · e2e 80/80 · CI 9 job 全绿（GitGuardian 曾因 `passwordUpdatedAt` 旁的 ISO 字面量判红，提成常量后转绿）
...
### PR 16B — 告警历史与统计 ✅（#192，2026-09-18 合入 main）

- [x] 新顶层导航「告警史」（只读，viewer+）：`getAlerts({status:"cleared"})` 展示已清除告警列表
      （发生/清除时间、时长、确认人、链到车辆）；空态在无 Mongo 时链到 系统状态
- [x] 就地统计：严重度分布、设备 Top-N、按天频次、确认率、时长均值/中位数（新 `CategoryBarChart`
      注册 ECharts BarChart；`lib/alertStats.ts` 纯函数）
- 统计前端算（后端零聚合基础设施，重度时间分桶聚合留给 17A 聚合层），受单次查询 500 条上限约束、
...
#### PR 16C-2 — 报码字典 ✅（#194，2026-09-18 合入 main）

- [x] **报码字典**（`code` → 名称 / 等级 / 分类 / 处理建议）配置化：内置基表 + 类型 + 纯合并/查找
      下沉 `@navfleet/shared`（`reportCodes.ts`），fleet-core 保留内置绑定封装（两套前端调用点不变）；
      部署侧 `codebook.json` 校验后叠加内置表，后端 `getCodebook()` 下发生效表
- [x] 管理页 `报码字典`（`/admin/codebook`，admin）：只读合并表 + 导出 JSON + 导入 JSON（客户端校验→
      `PUT /api/v1/codebook`，审计 `codebook_import`）；设备详情页改为按部署码表解读报码（新 `useCodebook`，
      未就绪回退内置表）
- [x] 字典热重载沿用 configRegistry 的原子替换 + 校验失败保留旧快照；`GET /codebook` viewer+、`PUT` admin
- 为承载导入落库，`deploy/docker-compose.yml` 的 config-runtime 卷 `read_only` → 可写（其余配置仍盘上手改）
- 边界：public 仓库不逐条抄厂商码表，只照抄 VDA 5050 `errorType` 公开枚举，其余按公开故障类别/分层
  自撰文案（真实形状非真实内容），真实码值等供应商给（见 P1-d 前置说明）

### PR 16D — 告警外发（拆成 16D-1 / 16D-2）

#### PR 16D-1 — 外发骨架 + HTTP 渠道 + 发送记录 ✅（#195，2026-09-19 合入 main）

- [x] 渠道（HTTP 三类）：webhook / 企业微信 / 钉钉，全局 `fetch` + `AbortSignal.timeout`、零新依赖，
      配小型重试+超时（5xx/429/网络错重试，4xx 不重试）；邮件（nodemailer）后置到 16D-2
- [x] 分级路由（**立即**）：各渠道按 `severity` 订阅 + 作用范围（复用规则引擎 `deviceMatchesScope`）；
      汇总/静默/升级留 16D-2
- [x] 挂 store 的 `alert.created`（告警 id 首次出现时恰好触发一次）第二个监听，fire-and-forget 不 await
      （事件同步跑在串行 ingest 队列里，await 慢渠道会卡住全部 ingest）；单实例下去重天然
- [x] 发送记录落库：独立 `notify_log` 集合（不塞 audit），镜像 audit 的建索引 + 内存 fallback + 吞错，
      无 Mongo 时 honest-empty；只读 admin API `GET /notify/log|config`（URL 打码，只给 `configured`）
- [x] 后端权威、盘上 `notify.json`：configRegistry 原子替换 + 校验失败保留旧快照 + 热重载；**红线**照搬
      Alertmanager「出厂空接收器」——出厂无文件=零渠道=不外发，渠道 URL 走 `urlEnv` 环境变量名、密钥不落盘
- 不单独发版（`feat` 累积到 release-please 的 1.3.0 PR，由 16D-2 收口）

#### PR 16D-2 — 投递策略 / 用户组 / 邮件 + 只读外发页 / 可观测性（拆成 16D-2a / 16D-2b）

##### PR 16D-2a — 邮件 + 用户组 + 汇总/静默/去重/升级（后端策略层）✅（#196，2026-09-19 合入 main）

- [x] 邮件渠道（nodemailer 直连 SMTP，新 `email` 类型）：SMTP 连接串仍走 `urlEnv` 环境变量、不落
      `notify.json`；`from` 走配置、收件人走 `recipients`/`groups`；与 HTTP 同形的重试+超时（`httpStatus` 恒 null）
- [x] 收件方用户组：`notify.json.groups` + `user:<用户名>`（发送时取 users 的 email，无邮箱者跳过），
      **只存在于 notify.json、不动 users/RBAC**；纯函数 `resolveRecipients`
- [x] 分级路由细化：warning 汇总（`digestSeconds`，critical 永远立即）+ 静默窗口（`silenceWindows`，跨零点+
      按星期）+ 去重加固（`renotifySeconds`）+ 升级策略（critical 未确认超时补发，确认/清除即取消）
- [x] 引入 nodemailer（lockfile 在 `node:22-alpine` 容器重算）；钉钉「加签」逐请求签名仍未支持（需第二个密钥）
- 不单独发版（`feat` 累积到 release-please 的 1.3.0 PR，由 16D-2b 收口）

##### PR 16D-2b — 只读外发页 + 可观测性（收口发版 **1.3.0**）✅（#197，2026-09-19 合入 main）

- [x] 只读控制台「外发」页 `/admin/notify`（admin）：生效渠道卡片（类型/启停/订阅严重度/configured 就绪，
      URL 服务端打码）+ 可筛选发送记录表（时间/设备/标题/渠道/严重度/状态/尝试/延时/错误，四态+URL 筛选+
      翻页）；零渠道=诚实空态；管理落地页加「外发」卡。fleet-core 加 `getNotifyLog`/`getNotifyConfig`。版式经负责人确认
- [x] 可观测性：Prometheus `navfleet_notify_sends_total{channel_type,status}` + `navfleet_notify_send_duration_seconds`
      直方图（NotifyService 送观察者，createApp 在 registry 建好后注入）；Grafana 面板补两块（成功/失败速率、按渠道 p95 延迟）
- [ ] **发版 1.3.0**：16A–16D 全部 feature 已在 main，release-please 的 1.3.0 发版 PR 待**负责人手动合并 + 打 tag**（收口动作）
...
### PR 17A — 聚合层（拆两段：17A-1 告警统计 → 17A-2 可用率/电量时序）

**17A-1 告警统计聚合 ✅（合入 main）**：`alerts` 集合上的服务端 `$facet` 聚合——严重度分布 /
设备 Top-N / 日频次 / 确认率 / 已清除告警的处理时长（均值+中位数），**不受 `/alerts` 那 500 条读取
上限约束**，即把 16B 的前端就地统计下沉到服务端（16B 注释已预告本阶段）。拆成纯管道 builder + 纯
mapper（各自单测——一条 `$group` key 写错、median 取错元素、未知严重度凭空多一档，机检不报但数字会
错），persistence 只负责「有 db 就 `.aggregate`、喂给 mapper；没 db 就诚实空态」，那层用 fake db 验
接线。无 Mongo = `available:false` 诚实空态（同 16B「无 Mongo 无历史」先例），而非把零值画成「一切
正常」。日频次按 `REPORT_TIMEZONE`（默认 `Asia/Shanghai`）切日界——UTC+8 的部署按 UTC 切日会把午夜后
的告警错算到"昨天"。新只读端点 `GET /reports/alerts`（viewer+，读，不动渠道/路由）+ fleet-core
`getAlertStatsReport`。**零新依赖**。

- **口径订正**：原列「schema 允许 `limit ≤ 5000`、服务端硬夹 500」这条**早在 14 系列已修**——
  `validation.ts` 现按 `config.maxHistoryPoints` 校验、openapi 由该 schema 生成（`history-limit.test.ts`
...
### PR 17B — 报表页与导出（拆两段：17B-1 报表页+CSV → 17B-2 定时报表）

**17B-1 报表页 + CSV 导出 ✅（合入 main）**：`/reports`（viewer+）消费 17A 两个聚合端点，版式经负责人
确认为 **B·顶部 KPI 带 + 两栏**——KPI 带（平均在线率/平均电量/告警总数/确认率）+ 左栏可用率趋势·电量
时序（`TimeSeriesChart`）+ 右栏告警严重度分布·设备 Top-N·按天频次（`CategoryBarChart`）。时间范围
（24h/7d/30d）/ 粒度（时/日）/ 设备三个 URL-backed 筛选；导出可用率/电量 CSV（`buildAvailabilityCsv`
纯函数，UTF-8 BOM 兼容 Excel 中文，零新依赖）。四态里无 Mongo → 两报表 `available:false` → 诚实空态
...
### PR 17C — 大屏值班模式（拆两段：17C-1 kiosk 认证 → 17C-2 大屏页）

**17C-1 kiosk 认证 ✅（合入 main）**：无人值守墙面屏要的「长期有效 + 只读 + 审计可辨识、不做匿名开放」
凭据。设计经负责人确认 = **专用 kiosk 账号 + 长 token**。落法（探查后取最小改动）：`UserRecord` 加可选
`kiosk` 标记（缺省=普通账号，**免迁移**），admin 建号时 `kiosk:true` **强制 role=viewer**（只读由 RBAC
天然保证，schema refine 拒绝非 viewer 的 kiosk）。长效只拉长**刷新期**：kiosk 账号的 refresh token 与
会话行寿命用 `KIOSK_REFRESH_TTL`（默认 180d），**access token 仍 15m**——吊销延迟不变。审计用现有
`detail` 字段（`login` / `user_create` 带 `{kiosk:true}`），无新 AuditAction。**吊销不变**：kiosk 一样
受 `tokenVersion` bump / 会话吊销 / 禁用即时失效（不豁免）。openapi + fleet-core 类型同步。

- **17C-2 大屏页 ✅（合入 main）**：`WallView` 建出——版式经确认 = **KPI 带 + 地图主体 + 滚动告警**。
  `meta.bare` 无壳（`App.vue` 已有分支），整屏 `pointer-events:none`（一面墙是被看的、不被操作的，顺带
  让地图自身的缩放按钮失效）。**新鲜度指示必备**：`wallFreshness` 把浏览器时钟测出的落后（非服务端时钟，
  否则会读出「-8 秒前」）每秒刷新，10s→黄、30s→红，配色标 token + 词（实时/数据延迟/画面可能已冻结），
  不靠颜色单独承载语义。纯逻辑（新鲜度 tone / KPI tile / worst-first 告警流）抽到 `lib/wallView.ts` 单测，
  视图只做薄渲染（同报表/图表的拆法，jsdom 不布局也不走时钟）。地图无 key 时走 `GpsMap` 自带的诚实占位。
  告警流溢出时才自动滚动（ResizeObserver 量出溢出、克隆列表 -50% 无缝循环，`prefers-reduced-motion` 归位
  顶部）；底部三分之一留白。多分辨率靠 `text-wall-*` + 2560 `wall:` 断点分级 + clamp 流式；`wall-drift`
  极缓慢位移做烧屏规避（reduced-motion 关）。UsersView 建号表单加「大屏 kiosk 账号」勾选（create-only、
