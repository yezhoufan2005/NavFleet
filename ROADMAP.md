...
### Phase 13 收口 — 还没完成，剩下的是核销而不是建设

四个页面与四个 tab 都建完了（13A–13F），但**收口的条件是 parity 清单全部勾掉**，而现在只有第 5 节
（历史回放）在 13E 里逐条核销过。剩下的第 1–4、6–8 节需要逐节比对新前端并写明改法 —— 那是一件
独立的、以"读旧实现 + 对新实现"为主的工作，不该塞进任何一个建设 PR 的尾巴里。
...
frontend **132** · console **443** · e2e **73**。
...
- [ ] **13T 之后把这条规律做成断言** —— 「store / composable 的每个导出都必须有非测试消费者」。
      它比人工逐行读 340 行可靠得多，而且能防止同类缺口再次出现

#### 要做的（结论明确，不需要再讨论）

- [ ] **告警数徽标回到导航**（第 1 节）。`summary.alertTotal` 在 store 里活着，外壳零消费。
      现在操作员在设备页 / 报表页 / 管理页**看不见有多少条告警在等他** —— 取消掉的是「不用切页
      就知道该不该切页」这个能力，是本批影响最大的一条
- [ ] **离线重试按钮**（第 1 节）。接上 `retryBootstrap`；`useNotifications` 已支持 action，
      toast 上挂一个即可。现在后端恢复后用户只能刷新页面
- [ ] **`<main>` 的焦点环**（第 1 节）。`AppShell.vue:130` 的 `focus-visible:outline-none` 与 9.9 的
...
- [ ] **骨架屏组件补回来，这是本批最大的一处**（第 7 节）。`frontend-next` 零 `*keleton*` 文件，
...
- [ ] **告警行的「来源」列与三项视觉编码**（第 6 节）。`alertSourceLabelMap` 四条映射在新前端无
...
- [ ] **「确认当前筛选」恢复为全集**（第 6 节）。现在缩成「确认本页」，而
      `docs/frontend-research.md:36` 对这条明确写的是「保持能力，补反馈与撤销」—— 反馈补了，能力窄了。
      缩窄**没有在代码或提交信息里给理由**
- [ ] **「清除已确认」控件 + 两个计数**（第 6/7 节）。接上 `clearAll` 与 `acknowledgedCount`。
      管理页那颗按钮不是等价物：它会连主题、侧栏、地图模式、声音偏好一起清掉
- [ ] **本地数据清除恢复按类粒度**（第 7 节）。接上 `clearSavedSceneViews`。**当前的「全清」会顺带
      删掉旧设置页刻意不碰的 theme / map-mode / device-layout / 声音偏好**，这是行为上的实质变化
- [ ] **「当前生效：深色/浅色」**（第 7 节）。`useTheme.ts` 的 `resolved` 已导出，只缺一个读者；
      选「跟随系统」时界面现在不告知此刻解析成了哪一套
- [ ] **搜索防抖**（第 6 节）。旧清单标的就是 🟡「加防抖」，现在每次按键还多一次 `router.replace`
- [ ] **GPS 标签的两条防御性 CSS**（第 4 节）：① `pointer-events:none` —— 现在 `opacity:0` 的标签
      仍可命中指针；② `hover` 时 `z-index:200` —— 旧 CSS 有注释专门说这是为了「密集车群里悬停出的
      标签不被邻居 pin 压住」。两条都是"平时看不出、密集时才出现"的那类规则

第 2/3/8 节核销追加的条目（同一批，同样按"结论明确"归类）：

- [ ] **设备列表补回「最近上报」与「电量」两列 + 行级视觉**（第 2.4 节）。**这是全批对值班效率影响
...
- [ ] **编队功能整段接上**（第 2.3 节）。这不是形态变化而是功能缺席：`selectFormation` /
...
- [ ] **`describeEnum` 的 13 条枚举释义接回 `title`**（第 2.6 节）。函数在 fleet-core 里活得很好、
      有测试，新前端零引用；控制模式 / 挡位 / 车端任务 / 平台任务四个字段的释义都没了
- [ ] **三处字段级损失**（第 2.6 节）：`speedLimit.stamp`（数据在 `fleetNormalize.ts:161`，界面无处
      显示 → **无法判断当前限速是刚下发的还是一小时前的残留**）· 报码卡的 `stamp`（`DescribedCode`
      结构里就没留这个字段，「这条报码何时发生」在详情页答不出）· 位姿分节空态的替代文案
- [ ] **「电量」格式化在仓库内不自洽**（第 2.6 节）：详情页 `(v,0," %")`、回放 tab `(v,1,"%")` ——
      同一台车两个 tab 显示不同精度。定一个，两处都改
- [ ] **世界底色的 `lanelet-mode` 分支**（第 3.9 节）。纯 lanelet 场景不再换底色，
      `--color-ros-lanelet-bg` 成了零消费者的 token
- [ ] **一批被静默简化的视觉编码**（第 3.9 节，全部没有注释说明）：lanelet 边界的 `round` cap ·
      选中轨迹的 `2 5` 虚线 + drop-shadow（**现在是实线**）· 僚车轨迹的 `2 6` 虚线 · 僚车标签的
      9px 粗体 + `paint-order: stroke` 描边光晕（浅色底图上少了对比补偿）。两处**语义**变化要一起定：
      僚车标记 `normal` 也被染成 brand，于是**「有颜色」不再等于「有状态」**；激光标记配色
      `--warning` → `--color-notice`
- [ ] **`dataDefaults` 的两个永久空常量**（第 8.7 节）。`sceneCatalog = {}` 与
      `fallbackFleetPayload.devices = []` 被原样照抄，三处查表恒不命中，离线兜底灌进去的是零台设备
      —— 也就是说**「后端不可达时有可视内容」这个承诺从 v1.0.0 起就没有实现过**。要么填示范数据、
      要么删掉这条路径并承认离线就是空白。**不要留着假装它有用**
- [ ] **删掉 store 里 4 个纯冗余再导出**（第 8.6 节）：`getDeviceTone` / `hasPose` / `round` /
      `formatDateTime` —— 所有消费者都直接从 `@navfleet/fleet-core` 导入。顺带给 `ingestPayload`
      的注释标明它是 test seam，否则下一个人会以为它是公共 API
- [ ] **修掉两处注释与实现的分叉**（核销中发现，改注释还是改代码要分别判断）：
...
- [ ] **补回 e2e「刷新后车辆仍 `<24px`」的断言**（第 3.5 节）。存储能力在，但钉住它的钉子松了 ——
      reload 用例现在只断言底图与版式偏好存活。「适应场景」的断言也从绝对 `>24px` 改成了相对比较

#### 要负责人先定的（我不替它推断理由）

- [ ] **CSS 动画整体缺席，是一个决定还是三次遗漏。** 新前端 `@keyframes` 零命中，而 v1.0.0 有三个，
      **三个都在编码「这东西是活的 / 这是此刻选中的那一个」**：`realtime-pulse`（顶栏状态点）·
      `pulse`（GPS 选中 marker）· `skeleton-sweep`（骨架屏）。对实时监控台来说「这数据是活的吗」
      几乎是最重要的问题，现在完全靠文字回答 —— 文字那层是净增强（`AppTopBar.vue:20-23` 的注释
      说得对：只变色调的点对色盲操作员什么也没说），**但前注意力通道被整个取消了**。ROS 图受影响
      最深：它本来就没有逐车选中样式，脉冲环是选中态唯一的动态表达。
      **所以下面三条（GPS pulse / ROS 脉冲环 / 骨架屏）应当合成一个决定，而不是三处分别补**
- [ ] **选中 marker 的 `pulse` 呼吸动画要不要回来**（第 4 节）。现在换成了静态强调
      （pin 12px→16px + `color-mix()` 外圈），**代码里没有注释说明为何去掉**
- [ ] **高德内置 `Scale` / `ToolBar` 是否仍要自绘**（第 4 节）。那行标着 🟡 改造，实际连位置参数都
      逐字照搬，仍是高德自己的皮肤，不受设计 token 与明暗主题影响
- [ ] **主题入口只剩会话菜单一个，是否接受**（第 7 节）。旧的可访问性理由已被满足（菜单里有显式
      三选项），但 header 一键切换没了。这更像 IA 决定而非遗漏
- [ ] **`registerWindowApi()` 是否刻意舍弃**（第 1 节）。旧版 6 个 window 上的调试/测试注入口在新
      前端不存在，也没有任何注释宣告废弃
- [ ] **统计卡的「当前场景」要不要回来**（第 2.1 节）。场景名现在只在地图浮层出现，列表与详情只显示
      裸 `sceneId`，`未配置场景` 这句降级文案全站搜不到
- [ ] **GpsMap 与 SceneMap 的「有意的不对称」是否还要**（第 2.5 节）。旧设计是 GpsMap 收全量、
      SceneMap 收编队筛选后；现在两者都吃 `filteredDevices`。因无编队选择 UI 而当前行为等价，
      **但设计意图丢了** —— 编队功能接回来之后这个差别会重新变得可见
