...
- [x] **远端分支清理已获授权并执行**：21 个已并入 `main` 的远端分支删除；
...
- [x] **vitest 5 与 Node 20：放弃 Node 20**（负责人授权我自行决定）。理由不是为了升 vitest ——
      **Node 20 已于 2026-04-30 EOL**，四个月来没有安全补丁。所以这不是「为了新版测试框架抬门槛」，
      而是我们本来就该做的事，vitest 5 的 `engines`（`^22.12.0 || ^24.0.0 || >=26.0.0`）只是把它顶到了眼前。
      分两步走，因为两步的风险完全不同：- **开发侧（下一个 PR）**：CI 矩阵 `[20, 22]` → `[22, 24]`、六处 `engines` 改 `>=22`，
      然后 `vitest` 与 `@vitest/coverage-v8` 同一个提交里抬到 5（拆开必定不解析）。
      不动任何交付物。- **交付侧（要跑一次容器闭环）**：**三个 Dockerfile 现在钉的是 `node:20-alpine`，
      也就是说我们此刻交付的镜像跑在一个已 EOL、不再有安全补丁的运行时上。**
      这是本轮顺带查出的问题，与 vitest 无关。建议改 `node:24-alpine`（当前 active LTS），
