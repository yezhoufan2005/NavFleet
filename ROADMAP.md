...
| **下一步**     | **Phase 16 进行中**：16A（确认落库，#190）、16B（告警历史与统计，#192）、16C-1（规则引擎收敛+配置化，#193）、16C-2（报码字典，#194）、16D-1（外发骨架+HTTP 渠道+发送记录，#195）、16D-2a（邮件+用户组+汇总/静默/去重/升级，#196）已合入 main。接着 16D-2b（只读外发页 + 可观测性）**收口发版 1.3.0**。16A–16D 用 `feat` 累积到 release-please 的 1.3.0 PR，合并与 tag 由负责人手动 |
...
##### PR 16D-2b — 只读外发页 + 可观测性（收口发版 **1.3.0**）

- [ ] 只读控制台「外发」页：发送记录 + 生效渠道概览（消费 16D-1 的 `GET /notify/*`，密钥打码）；版式先与负责人确认再建
