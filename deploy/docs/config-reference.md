...
| `RATE_LIMIT_MAX`            | `600`                  | 窗口内每 IP 的 `/api` 请求上限；超限返回 `429 {"error":"too_many_requests"}`                                                               |
| `AUTH_RATE_LIMIT_WINDOW_MS` | `900000`               | `/api/auth` 凭据接口的限流窗口（默认 15 分钟），与全局限流叠加                                                                             |
| `AUTH_RATE_LIMIT_MAX`       | `50`                   | 该窗口内每 IP 的凭据请求上限。共用出口 IP 的办公网、或每个用例都登录的测试套件可能需要放宽                                                 |
| `AUTH_LOCK_THRESHOLD`       | `5`                    | 账号级登录锁定阈值：单个账号连续失败这么多次即锁定。与每 IP 限流正交，防单账号被多来源轮换爆破                                             |
| `AUTH_LOCK_WINDOW_MS`       | `900000`               | 锁定持续时长（默认 15 分钟）。锁定期内即使口令正确也拒绝，且不向客户端透露账号是否被锁；过期后计数清零                                     |
